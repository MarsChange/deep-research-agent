...
if os.getenv("PLAYWRIGHT_MCP_URL"):
    try:
        ...
        agent_logging.warning(f"Browser tools unavailable (missing dependency): {e}")
        BROWSER_TOOLS = []
...
if os.getenv("DASHSCOPE_API_KEY") and os.getenv("JINA_API_KEY"):
    try:
        ...
        agent_logging.warning(f"Webpage analyzer tools unavailable (missing dependency): {e}")
        WEBPAGE_ANALYZER_TOOLS = []
...
if os.getenv("E2B_API_KEY"):
    try:
        ...
        agent_logging.warning(f"Sandbox tools unavailable (missing dependency): {e}")
        SANDBOX_TOOLS = []
...
if os.getenv("SERPAPI_API_KEY"):
    try:
        ...
        agent_logging.warning(f"Search tools unavailable (missing dependency): {e}")
        SEARCH_ENGINE_TOOLS = []
...
try:
    ...
    agent_logging.warning(f"Wiki search tools unavailable (missing dependency): {e}")
    WIKI_SEARCH_TOOLS = []
...
if os.getenv("JINA_API_KEY"):
    try:
        ...
        agent_logging.warning(f"Scrape website tools unavailable (missing dependency): {e}")
        SCRAPE_WEBSITE_TOOLS = []
...
TOOLS = SUB_AGENT_TOOLS + MAIN_AGENT_TOOLS
__all__ = [
    "TOOLS",
    "SUB_AGENT_TOOLS",
    "MAIN_AGENT_TOOLS",
